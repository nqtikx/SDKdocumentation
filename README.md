...
| `onPayment` | A callback function triggered during the on-ramp process when payment information becomes available. Receives `{ transactionId, orderId }` where `transactionId` is the payment provider transaction id and `orderId` is the internal order id. Use it to track payment status, store transaction data, or trigger backend/UI updates. |
